...
class DashboardApp:
    ...
    def render_clock(self):
        ...
        self.display.draw_text_centered(day_str_br, 15, self.font_path, 11, fill=(255, 200, 50))
        ...
        # Dia em Inglês (Azul Claro)
        self.display.draw_text_centered(day_str_en, 88, self.font_path, 10, fill=(150, 220, 255))
        
        self.display.draw_line(108, margin=20, fill=(150, 220, 255))
        
        # Data no rodapé (Ciano Claro)
        self.display.draw_text_centered(date_str, 120, self.font_path, 13, fill=(200, 255, 255))
    ...
    def render_weather(self):
        ...
        self.display.draw_text_centered(f"{temp}°C", 45, self.font_path, 38, fill=color)
        
        self.display.draw_text_centered(desc, 85, self.font_path, 10, fill=(200, 255, 255))
        
        self.display.draw_line(105, margin=20, fill=(200, 255, 255))
        
        self.display.draw_text_centered(f"SENSAÇÃO: {feels}°C", 118, self.font_path, 10, fill=(255, 255, 200))
        self.display.draw_text_centered(f"UMIDADE: {humi}%", 135, self.font_path, 10, fill=(255, 255, 200))
    ...
    def render_spotify(self, spotify):
        ...
        self.display.draw_text_centered("SPOTIFY", 32, self.font_path, 10, fill=(30, 255, 120))
        self.display.draw_line(45, margin=30, fill=(30, 255, 120))
        
        music_name = spotify['music']
        if len(music_name) > 16: music_name = music_name[:14] + ".."
        self.display.draw_text_centered(music_name.upper(), 65, self.font_path, 13, fill=(255, 255, 255))
        
        artist_name = spotify['artists']
        if len(artist_name) > 22: artist_name = artist_name[:20] + ".."
        self.display.draw_text_centered(artist_name, 90, self.font_path, 11, fill=(200, 255, 255))
        ...
        self.display.draw_text_centered(status_text, 140, self.font_path, 9, fill=(255, 255, 200))
    ...
    def render_printer(self, printer):
        ...
        # Cabeçalho
        self.display.draw_text_centered("K1 MAX", 10, self.font_path, 15, fill=(255, 140, 0))
        ...
        if state in ["PRINTING", "PAUSED"]:
            ...
            self.display.draw_text_centered(f"{progress_pct}%", 45, self.font_path, 40, fill=(255, 255, 255))
            ...
            self.display.draw_text_centered(info_text, 125, self.font_path, 12, fill=(200, 255, 255))
        else:
            # --- TELA IDLE (CRIATIVA) ---
            # Status Principal
            ...
            self.display.draw_text_centered("TEMPERATURAS", 85, self.font_path, 10, fill=(200, 255, 255))
            
            # Bico (Extruder)
            temp_ext = int(printer.get('temp_extruder', 0))
            self.display.draw_text_centered(f"BICO: {temp_ext}°C", 105, self.font_path, 15, fill=(255, 100, 50))
            
            # Mesa (Bed)
            temp_bed = int(printer.get('temp_bed', 0))
            self.display.draw_text_centered(f"MESA: {temp_bed}°C", 130, self.font_path, 15, fill=(50, 150, 255))
    ...
    def render_system(self):
        ...
        # Título
        self.display.draw_text_centered("RASPBERRY PI", 10, self.font_path, 12, fill=(255, 50, 80))
        ...
        self.display.draw_text_centered("CPU TEMP", 85, self.font_path, 11, fill=(200, 255, 255))
        ...
        # Carga do Sistema
        self.display.draw_text_centered(f"LOAD: {int(stats['cpu'])}%", 138, self.font_path, 10, fill=(255, 255, 200))

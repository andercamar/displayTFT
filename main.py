...
class DashboardApp:
    ...
    def render_clock(self):
        ...
        self.display.draw_text_centered(day_str_br, 15, self.font_path, 18, fill=(255, 200, 50))
        
        # Hora Central (Mais para o meio agora que não tem ícone no topo)
        self.display.draw_text_centered(time_str, 42, self.font_path, 48, fill=(255, 255, 255))
        
        # Dia em Inglês (Branco Puro)
        self.display.draw_text_centered(day_str_en, 92, self.font_path, 14, fill=(255, 255, 255))
        
        self.display.draw_line(112, margin=20, fill=(255, 255, 255))
        
        # Data no rodapé (Branco Puro)
        self.display.draw_text_centered(date_str, 128, self.font_path, 18, fill=(255, 255, 255))

        # Barra Pulsante
        pulse_color = (0, 255, 0) if int(now.strftime('%S')) % 2 == 0 else (0, 180, 0)
    ...
    def render_weather(self):
        ...
        self.display.draw_text_centered(f"{temp}°C", 45, self.font_path, 48, fill=color)
        
        self.display.draw_text_centered(desc, 85, self.font_path, 16, fill=(255, 255, 255))
        
        self.display.draw_line(105, margin=20, fill=(255, 255, 255))
        
        self.display.draw_text_centered(f"SENSAÇÃO: {feels}°C", 118, self.font_path, 14, fill=(255, 255, 255))
        self.display.draw_text_centered(f"UMIDADE: {humi}%", 138, self.font_path, 14, fill=(255, 255, 255))
    ...
    def render_spotify(self, spotify):
        ...
        self.display.draw_text_centered("SPOTIFY", 32, self.font_path, 16, fill=(30, 255, 120))
        ...
        self.display.draw_text_centered(artist_name, 90, self.font_path, 16, fill=(255, 255, 255))
        ...
        self.display.draw_text_centered(status_text, 140, self.font_path, 14, fill=(255, 255, 255))
    ...
    def render_printer(self, printer):
        ...
        if state in ["PRINTING", "PAUSED"]:
            ...
            self.display.draw_text_centered(f"{progress_pct}%", 45, self.font_path, 48, fill=(255, 255, 255))
            ...
            self.display.draw_text_centered(state, 45, self.font_path, 28, fill=status_color)
            ...
            self.display.draw_text_centered(f"BICO: {temp_ext}°C", 110, self.font_path, 18, fill=(255, 100, 50))
            
            # Mesa (Bed)
            temp_bed = int(printer.get('temp_bed', 0))
            self.display.draw_text_centered(f"MESA: {temp_bed}°C", 135, self.font_path, 18, fill=(50, 150, 255))
    ...
    def render_system(self):
        ...
        # Título
        self.display.draw_text_centered("RASPBERRY PI", 10, self.font_path, 16, fill=(255, 50, 80))
        ...
        self.display.draw_text_centered(f"{int(temp)}°C", 45, self.font_path, 48, fill=temp_color)
        self.display.draw_text_centered("CPU TEMP", 88, self.font_path, 16, fill=(255, 255, 255))
        
        # Uso de RAM
        ram_pct = stats['ram_usage']
        self.display.draw_text_centered(f"RAM: {int(ram_pct)}%", 112, self.font_path, 16, fill=(255, 255, 255))
        self.display.draw_progress_bar(ram_pct/100, 126, height=4, color=(50, 180, 255))
        
        # Carga do Sistema
        self.display.draw_text_centered(f"LOAD: {int(stats['cpu'])}%", 140, self.font_path, 14, fill=(255, 255, 255))
